...
tuple<int, int, int> GetNextPos(int x, int y, int dir) {
	...
	if (InRange(nx, ny) && make_pair(nx, ny) != make_pair(cat_x, cat_y)) return make_tuple(nx, ny, dir);
	else {
		for (int i = 1; i < DIR_NUM; i++) {
			int nx = x + dx[(dir + i) % 8], ny = y + dy[(dir + i) % 8];
			if (InRange(nx, ny) && make_pair(nx, ny) != make_pair(cat_x, cat_y)) {
